...
class TopKChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 将TopK通道的权重设为1（也可以设置为通道的统计值）
        weights.scatter_(1, top_indices, 1.0)  # 硬选择

        # 也可以使用软选择，将TopK通道的权重设为其统计值
        # weights.scatter_(1, top_indices, torch.gather(y, 1, top_indices))
...
def generate_ordered_heatmaps(features_dict, original_img, save_dir=None, top_k=32):
    ...
    for i, (key, title, feature_key) in enumerate(display_order):
        ...
        ax.set_title(f'{title}\n(Top{min(top_k, C)}通道注意力)', fontsize=10)
        ax.axis('off')
...
if __name__ == "__main__":
    ...
    # 请修改为你的实际路径
    # --------------------------
    img_path = "/amax/zs/code/WSCC_TAF-main/datasets/ShanghaiTech/part_A_final/test_data/images/IMG_11.jpg"  # 替换为你的图像路径
    model_path = "/amax/zs/code/WSCC_TAF-main/save_file/ShanghaiA_swincdpnet3/model_best.pth"  # 替换为你的模型权重路径
    save_dir = "./results_topk_attention"  # 结果保存目录
    # top_k = 32  # 选择的TopK通道数
    top_k = 12  # 选择的TopK通道数

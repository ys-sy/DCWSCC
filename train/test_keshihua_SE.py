...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.shape
        y = self.avg_pool(x).view(b, c)  # 提取通道统计信息
        y = self.fc(y).view(b, c, 1, 1)  # 学习权重
        return y  # 输出形状: (b, c, 1, 1)
...
def load_model_with_hooks(model_path):
    ...
    if len(model.layers) >= 4:
        model.layers[0].register_forward_hook(
            lambda m, i, o: intermediate_features.update({'x1': o[1].detach()})
        )
        model.layers[1].register_forward_hook(
            lambda m, i, o: intermediate_features.update({'x2': o[1].detach()})
        )
        model.layers[2].register_forward_hook(
            lambda m, i, o: intermediate_features.update({'x3': o[1].detach()})
        )
        model.layers[3].register_forward_hook(
            lambda m, i, o: intermediate_features.update({'x4': o[1].detach()})
        )
    else:
        raise ValueError("模型layers数量不足4层，无法获取x1-x4特征")
...
# 5. 热力图生成与显示
# ------------------------------
def generate_ordered_heatmaps(features_dict, original_img, save_dir=None, reduction=16):
    """按顺序生成并显示所有热力图"""
    display_order = [
        ('original', '原图', None),
        ('x1', '第一阶段特征', 'x1'),
    ...
    for i, (key, title, feature_key) in enumerate(display_order):
        ...
        # 初始化注意力模块
        attention = ChannelAttention(in_channels=C, reduction=reduction).to(device)
        attention.eval()
    ...
    if save_dir:
        ...
        save_path = os.path.join(save_dir, 'attention_heatmaps.png')
...
# 6. 主函数
# ------------------------------
def main(img_path, model_path, save_dir=None, reduction=16):
    """主函数：生成所有层的热力图"""
    # 预处理图像
    print("预处理图像...")
...
if __name__ == "__main__":
    ...
    # 请修改为你的实际路径
    # --------------------------
    img_path = "/amax/zs/code/WSCC_TAF-main/datasets/ShanghaiTech/part_A_final/test_data/images/IMG_137.jpg"  # 替换为你的图像路径
    model_path = "/amax/zs/code/WSCC_TAF-main/save_file/ShanghaiA_swincdpnet3/model_best.pth"  # 替换为你的模型权重路径
    save_dir = "./results_attention"  # 结果保存目录
    reduction = 16  # 注意力模块降维比例（8~32）

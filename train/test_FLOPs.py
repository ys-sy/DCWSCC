...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.input_resolution = input_resolution
        self.depth = depth
        self.use_checkpoint = use_checkpoint  # 保留这个参数用于控制是否使用checkpoint
    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                # 修正：明确使用torch.utils.checkpoint.checkpoint
...
# 1. 设置模型路径（替换为你的.pt文件路径）
model_path = "/amax/zs/code/WSCC_TAF-main/save_file/ShanghaiA_swincdpnet3/model_best.pth"  # 替换为实际路径
...
# 5. 创建输入张量 (batch_size=1, channels=3, height=384, width=384)

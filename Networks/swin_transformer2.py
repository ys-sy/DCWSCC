...
class SwinTransformer2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.mode == 3:
            self.post_conv = nn.Sequential(
                # 使用self.num_features作为输入通道数，与Swin输出匹配
                FusedRes_SE2D(self.num_features, 1024, stride=1),
            )
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C（3维：Batch, Length, Channel）

        if self.mode == 0:
            x = self.avgpool(x)  # B L 48
        elif self.mode == 1:
            x = self.avgpool(x.transpose(1, 2))  # 处理3维输入
        elif self.mode == 2 or self.mode == 3:
            # 1. 转换为3维（B, C, L）：将通道维放到第二维
            x = x.transpose(1, 2)  # 形状：(B, C, L)，此时是3维
            B, C, L = x.shape  # 解包3维形状

            # 2. 计算H和W（确保L = H * W）
            H = int(L ** 0.5)  # 假设特征图是正方形
            W = L // H  # 避免因L非完全平方数导致的错误

            # 3. 转换为4维张量（B, C, H, W）
            x = x.view(B, C, H, W)  # 现在是4维

            # 4. 执行后续操作（此时x是4维，可正常处理）
            x = self.post_conv(x)  # B C H W
            x = self.avgpool(x)  # B C 1 1（2D池化适配4维输入）
            x = x.view(B, -1)  # 展平为B C
        elif self.mode == 4:
            # 若 mode=4 涉及 4D 特征，同样使用 2D 池化
            x = x.transpose(1, 2)
            B, C, H, W = x.shape
            x = x.view(B, C, H, W)
            x1 = self.post_conv1(x)
            x1 = self.avgpool1(x1).view(B, -1)  # 展平为 B C
            x2 = self.post_conv2(x)
            x2 = self.avgpool2(x2).view(B, -1)
            return x1, x2
...
#             idct_matrix = self._build_idct_matrix(max_len)
...
#
#     def _build_idct_matrix(self, n):
#         """构建离散余弦反变换矩阵"""
#         # IDCT矩阵是DCT矩阵的转置，并进行适当缩放
#         idct_m = np.zeros((n, n), dtype=np.float32)
#         for k in range(n):
#             for i in range(n):
#                 if i == 0:
#                     idct_m[k, i] = 1.0 / np.sqrt(n)
#                 else:
#                     idct_m[k, i] = np.cos(np.pi * i * (2 * k + 1) / (2 * n)) * np.sqrt(2.0 / n)
#         return torch.from_numpy(idct_m)
#
#     def forward(self, x):
#         """应用DCT变换"""
#         b, c, l = x.size()
...
#         y = self.fc(y).view(b, c, 1)
#         x_att = x_freq * y.expand_as(x_freq)
#
#         # 关键修改：通过IDCT反变换将频域信息转回空间域
#         x_spatial = self.dct.inverse(x_att)
#
#         # 与原始空间域信息残差连接
...
class DCT2D(nn.Module, metaclass=SingletonMeta):
    """2D离散余弦变换模块（单例模式）"""

    def __init__(self, max_h=256, max_w=256):
        ...
        if not hasattr(self, '_initialized'):
            # 构建水平和垂直方向的DCT矩阵
            dct_h = self._build_dct_matrix(max_h)
            dct_w = self._build_dct_matrix(max_w)
            # 构建IDCT矩阵（转置并缩放）
            idct_h = self._build_idct_matrix(max_h)
            idct_w = self._build_idct_matrix(max_w)

            # 注册缓冲区，使用安全的方式检查是否已存在
            if 'dct_h' not in self._buffers:
                self.register_buffer('dct_h', dct_h)
            if 'dct_w' not in self._buffers:
                self.register_buffer('dct_w', dct_w)
            if 'idct_h' not in self._buffers:
                self.register_buffer('idct_h', idct_h)
            if 'idct_w' not in self._buffers:
                self.register_buffer('idct_w', idct_w)

            self._initialized = True
            print(f"DCT2D initialized with max_h={max_h}, max_w={max_w}")
    ...
    def forward(self, x):
        """2D DCT变换: 先对行变换，再对列变换"""
        b, c, h, w = x.size()
        h_use, w_use = min(h, self.max_h), min(w, self.max_w)

        # 对行应用DCT
        x_flat = rearrange(x, 'b c h w -> (b c) h w')  # (b*c)×h×w
        x_row = torch.matmul(self.dct_h[:h_use, :h_use], x_flat)  # (b*c)×h×w

        # 对列应用DCT
        x_col = torch.matmul(x_row, self.dct_w[:w_use, :w_use])  # (b*c)×h×w
        return rearrange(x_col, '(b c) h w -> b c h w', b=b, c=c)  # 恢复BCHW格式

    def inverse(self, x):
        """2D IDCT反变换: 先对列反变换，再对行反变换"""
        b, c, h, w = x.size()
        h_use, w_use = min(h, self.max_h), min(w, self.max_w)

        # 对列应用IDCT
        x_flat = rearrange(x, 'b c h w -> (b c) h w')  # (b*c)×h×w
        x_col = torch.matmul(x_flat, self.idct_w[:w_use, :w_use])  # (b*c)×h×w

        # 对行应用IDCT
        x_row = torch.matmul(self.idct_h[:h_use, :h_use], x_col)  # (b*c)×h×w
        return rearrange(x_row, '(b c) h w -> b c h w', b=b, c=c)  # 恢复BCHW格式


class FrequencyBandingReorganization2D(nn.Module):
    """2D频谱分带重组模块"""

    def __init__(self, bands=4):
        super(FrequencyBandingReorganization2D, self).__init__()
        self.bands = bands

    def forward(self, x):
        """
        输入: B×C×H×W
        输出: B×bands×(C//bands)×H×W
        """
        b, c, h, w = x.size()
        assert c % self.bands == 0, "通道数必须能被分带数整除"
        return rearrange(x, 'b (bn cbn) h w -> b bn cbn h w', bn=self.bands)


class MultiScaleFrequencyAttention2D(nn.Module):
    """2D多尺度频域注意力模块（修正版）"""

    def __init__(self, bands=4, planes=None):
        super(MultiScaleFrequencyAttention2D, self).__init__()
        self.bands = bands
        self.planes = planes

        # 多尺度2D卷积（分组卷积，输入输出通道数=planes）
        self.conv3 = nn.Conv2d(
            planes, planes,
            kernel_size=3, padding=1,
            groups=bands  # 分组数=bands，确保planes能被bands整除
        )
        ...
        # 修正门控层：输入通道数=2×bands（而非2×planes）
        self.gate = nn.Sequential(
            nn.Conv2d(2 * bands, bands, kernel_size=1),  # 输入=2×bands，输出=bands
            nn.Sigmoid()
        )

    def forward(self, x):
        b, bn, cbn, h, w = x.size()  # bn=bands, cbn=planes//bands

        # 展平为 B×(bands×C_b)×H×W（即B×planes×H×W）
        x_flat = rearrange(x, 'b bn cbn h w -> b (bn cbn) h w')

        # 多尺度特征提取
        x3 = self.conv3(x_flat)  # B×planes×H×W
        x5 = self.conv5(x_flat)  # B×planes×H×W

        # 恢复为 B×bands×C_b×H×W 格式
        x3 = rearrange(x3, 'b (bn cbn) h w -> b bn cbn h w', bn=bn)
        x5 = rearrange(x5, 'b (bn cbn) h w -> b bn cbn h w', bn=bn)

        # 拼接x3和x5，形状变为 B×(2×bands)×C_b×H×W
        gate_input = torch.cat([x3, x5], dim=1)  # [b, 2*bn, cbn, h, w]

        # 压缩cbn维度（消除cbn标识符）
        gate_input = torch.mean(gate_input, dim=2, keepdim=True)  # [b, 2*bn, 1, h, w]

        # 重组为 B×(2×bands)×H×W
        gate_input_flat = rearrange(gate_input, 'b bn2 1 h w -> b bn2 h w', bn2=2 * bn)  # [b, 8, 12, 12]

        # 计算注意力权重（输入通道=8，与gate的conv2d输入通道2×bands=8匹配）
        g = self.gate(gate_input_flat)  # [b, bands, h, w]
        g = rearrange(g, 'b bn h w -> b bn 1 h w')  # 扩展维度用于加权

        return g * x5 + (1 - g) * x3  # 注意力加权融合


class FusedFrequencySELayer2D(nn.Module):
    """2D频域增强SE模块（修正版）"""

    def __init__(self, channel, bands=4, reduction=4, max_dct_h=256, max_dct_w=256):
        super(FusedFrequencySELayer2D, self).__init__()
        self.channel = channel
        self.bands = bands
        self.dct = DCT2D(max_h=max_dct_h, max_w=max_dct_w)
        self.sbr = FrequencyBandingReorganization2D(bands=bands)

        # 传入通道数channel到注意力模块
        self.ms_fa = MultiScaleFrequencyAttention2D(bands=bands, planes=channel)
    ...
    def forward(self, x):
        """
        输入: B×C×H×W (空间域特征)
        输出: B×C×H×W (增强后的空间域特征)
        """
        residual = x  # 保留原始空间特征
        b, c, h, w = x.size()

        # 1. 空间域 -> 频域
        x_dct = self.dct(x)  # B×C×H×W (频域特征)

        # 2. 频谱分带重组
        x_bands = self.sbr(x_dct)  # B×bands×(C//bands)×H×W

        # 3. 多尺度频域注意力
        x_fused = self.ms_fa(x_bands)  # B×bands×(C//bands)×H×W
        x_freq = rearrange(x_fused, 'b bn cbn h w -> b (bn cbn) h w')  # 恢复通道维度 B×C×H×W

        # 4. 频域注意力加权
        y = self.avg_pool(x_freq).view(b, c)  # B×C (全局频域统计)
        y = self.fc(y).view(b, c, 1, 1)  # B×C×1×1 (注意力权重)
        x_att = x_freq * y.expand_as(x_freq)  # 频域特征加权

        # 5. 频域 -> 空间域 (关键：通过IDCT转回空间域)
        x_spatial = self.dct.inverse(x_att)  # B×C×H×W

        # 6. 残差连接：空间域原始特征 + 频域增强特征
        return residual + x_spatial


class FusedRes_SE2D(nn.Module):
    """融合频域分带的2D残差模块 (BCHW输入格式)"""

    def __init__(self, inplanes, planes, stride=1, use_se=True,
                 bands=4, reduction=4, max_dct_h=256, max_dct_w=256):
        ...
        self.use_se = use_se
        if use_se:
            self.se = FusedFrequencySELayer2D(
                planes,
                bands=bands,
                reduction=reduction,
                max_dct_h=max_dct_h,
    ...
    def forward(self, x):
        """
        输入: B×C_in×H×W
        输出: B×C_out×H'×W' (H'=H/stride, W'=W/stride)
        """
        residual = self.downsample(x)  # 残差分支

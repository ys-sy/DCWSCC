...
class LlamaMLP(nn.Module):
    def __init__(self, hidden_size, intermediate_size):
        ...
        assert intermediate_size is not None, "intermediate_size is None"
        print(f"[LlamaMLP] hidden_size={hidden_size}, intermediate_size={intermediate_size}")
...
class LLMLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        print(f"[LLMLayer] dim={dim}, inner_dim={inner_dim} (type={type(inner_dim)})")
...
class LLMBottleneck(nn.Module):
    def __init__(
        ...
        print(f"[LLMBottleneck] hidden_size={hidden_size}, mlp_ratio={mlp_ratio}, intermediate_size={intermediate_size}")
    ...
    def forward(self, x):
        ...
        # 如果前面是序列格式，不能 reshape 成图像
        return x


class SwinTransformer_LLM(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.mode == 3:
            self.post_conv = nn.Sequential(
                ...
                FusedRes_SE1D(768, 1024, stride=1),  # B, 1024, 284
            )
        elif self.mode == 2:
            self.post_conv = nn.Sequential(
        ...
        # 添加 LLMBottleneck 层
        self.llm_bottleneck = LLMBottleneck(
            in_dim=self.num_features,
            mlp_ratio=mlp_ratio,
            hidden_size=768,
            num_heads=8,
            n_layers=2,
            attention_method="hyper"  # 可以根据需要修改
        )

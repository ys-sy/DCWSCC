...
class DCT1D(nn.Module, metaclass=SingletonMeta):
    ...
    def forward(self, x):
        """应用DCT变换，适配输入长度与DCT矩阵维度"""
        b, c, l = x.size()  # x形状：(B, C, L)
        use_len = min(l, self.max_len)

        # 裁剪输入特征的长度维度至use_len（关键修复）
        x_cropped = x[..., :use_len]  # 保留前use_len长度的特征

        # 获取对应长度的DCT矩阵
        dct_matrix = self.dct_matrix[:use_len, :use_len]

        # 执行矩阵乘法（此时x_cropped最后一维为use_len，与dct_matrix匹配）
        return torch.matmul(x_cropped, dct_matrix)
...
class SwinTransformer_MULTI(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # 新增：stage2的通道映射与上采样（仅用于mode=3）
        self.upsample2 = nn.Sequential(
            nn.Conv2d(int(embed_dim * 2 ** 1), self.num_features, kernel_size=1, bias=False),
            # 第2阶段通道数为embed_dim×2^1，映射到num_features
            nn.BatchNorm2d(self.num_features)
            # 不需要上采样，因为stage2的尺寸已经是目标尺寸
        )

        # 原有upsample3和upsample4保持不变
        self.upsample3 = nn.Sequential(
            nn.Conv2d(int(embed_dim * 2 ** 2), self.num_features, kernel_size=1, bias=False),
            nn.BatchNorm2d(self.num_features),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        )
        self.upsample4 = nn.Sequential(
            nn.Conv2d(int(embed_dim * 2 ** 3), self.num_features, kernel_size=1, bias=False),
            nn.BatchNorm2d(self.num_features),
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        )
        # 修正融合卷积：输入输出均为num_features
        self.fusion_conv = nn.Conv2d(self.num_features, self.num_features, kernel_size=3, padding=1, bias=False)
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            stage_outputs.append(x_before_downsample)

        # 对于mode=3，进行特征融合
        if self.mode == 3 and len(stage_outputs) >= 4:
            # 获取各阶段特征
            stage2 = stage_outputs[1]  # 第2阶段特征
            stage3 = stage_outputs[2]  # 第3阶段特征
            stage4 = stage_outputs[3]  # 第4阶段特征

            # 重塑特征为2D格式（B, C, H, W）
            B, _, C2 = stage2.shape
            H2 = W2 = int(math.sqrt(stage2.shape[1]))  # 第2阶段特征图尺寸（如28×28）
            stage2_2d = stage2.permute(0, 2, 1).view(B, C2, H2, W2)  # 形状：(B, 192, 28, 28)

            # 新增：将stage2_2d的通道映射为num_features
            stage2_up = self.upsample2(stage2_2d)  # 形状：(B, num_features, 28, 28)

            # 处理第3阶段特征（通道映射+上采样）
            B3, _, C3 = stage3.shape
            H3 = W3 = int(math.sqrt(stage3.shape[1]))  # 第3阶段特征图尺寸（如14×14）
            stage3_2d = stage3.permute(0, 2, 1).view(B3, C3, H3, W3)
            stage3_up = self.upsample3(stage3_2d)  # 形状：(B, num_features, 28, 28)

            # 处理第4阶段特征（通道映射+上采样）
            B4, _, C4 = stage4.shape
            H4 = W4 = int(math.sqrt(stage4.shape[1]))  # 第4阶段特征图尺寸（如7×7）
            stage4_2d = stage4.permute(0, 2, 1).view(B4, C4, H4, W4)
            stage4_up = self.upsample4(stage4_2d)  # 形状：(B, num_features, 28, 28)

            # 特征融合（通道数已统一为num_features）
            fused_feature = stage2_up + stage3_up + stage4_up  # 关键修复：通道维度匹配
            fused_feature = self.fusion_conv(fused_feature)  # 融合后卷积

            # 重塑为序列特征格式
            fused_feature = fused_feature.view(B, self.num_features, H2 * W2).permute(0, 2, 1).contiguous()

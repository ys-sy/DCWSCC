...
transform = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.485, 0.456, 0.406],  # 标准化（ImageNet均值）
...
def load_and_process_image(image_path):
    ...
    plt.close()
    print("Original image saved to 'visualizations/original_image.png'")
    ...
    features = img_to_1d(img_tensor)
    print(f"1D features shape: {features.shape}")
...
if __name__ == "__main__":
    # 替换为你的图像路径
    # 例如：image_path = "my_image.jpg"
    # image_path = input("请输入你的图像路径（例如：my_photo.jpg）: ").strip()

    image_path = "/amax/zs/code/WSCC_TAF-main/datasets/partA_Estimated_101.jpg"

...
class FusedFrequencySELayer1D(nn.Module):
    ...
    def __init__(self, channel, bands=4, reduction=4, max_dct_len=1024):
        ...
        self.bands = bands
        self.dct = DCT1D(max_len=max_dct_len)  # 单例DCT实例
...
class SwinTransformer_CDPNET3(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # mode=3 特征处理
        x = self.post_conv(x.transpose(1, 2))  # 转置为B C L，通过频域残差模块
        x = self.avgpool(x)  # 自适应池化到B C 1
        x = torch.flatten(x, 1)  # 展平为B C
        return x
